...
export function parseSeriesName(name:string):string {
   ...
   let mname:string[] = name.split('.')
    mname = mname.filter(d=>!(d.toLowerCase()=="dub" || d.toLowerCase() == "sub"))
    name = mname.join(' ')
return name;
}
export async function getJellyfinStatus(): Promise<string | null> {
const jellyfinData = await fetch(process.env.MY_JELLYFIN_INSTANCE + "/Sessions?active=true", {
headers: {
    "X-Emby-Token": process.env.MY_JELLYFIN_TOKEN || "no"
}
}).then(r=>r.json())
const mySession = jellyfinData.find(d=>d.NowPlayingItem) as JellyfinSession
...
const title = isMovie ? mySession.NowPlayingItem.Name : `${mySession.NowPlayingItem.SeriesName} - ${parseSeriesName(mySession.NowPlayingItem.Name)}`
