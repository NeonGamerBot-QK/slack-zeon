...
export function getKudosMoments() {
    return fetch('https://juice.hackclub.com/api/get-omg-moments').then(r=>r.json() as Promise<Moment[]>)
}
...
export async function cron(app: ModifiedApp) {
    ...
    for(const moment of moments) {
        ...
        await fetch(moment.video).then(r=>r.arrayBuffer()).then(Buffer.from).then(d=>{
