...
function extractText(t:string):Schedule[] {
  ...
  splits = splits.slice(splits.findIndex(x => x.includes('schedule')) + 1).filter(Boolean);
...
export async  function sendSchedule(text:string, app: ModifiedApp, channel: string) {
