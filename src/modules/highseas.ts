...
const all_users = []
const page0 = await fetch("https://doubloons.cyteon.hackclub.app/api/v1/data?page=1").then(r => r.json())
const pages = page0.pages
for (let i = 0; i < pages; i++) {
    const page = fetch(`https://doubloons.cyteon.hackclub.app/api/v1/data?page=${i + 1}`).then(r => r.json()).then(r=>r.users)
    all_users.push(page)
}
