...
export default class JoinManager implements Command {
  ...
  run(app: ModifiedApp) {
    ...
    app.command("/joinchannel", async ({ command, ack, respond }) => {
      ...
      const userMatch = targetUserText.includes("<@") ? targetUserText.split("<@")[1].split(">")[0] : targetUserText;
    ...
    app.command(this.name, async ({ command, ack, respond }) => {
      ...
      switch (action) {
        ...
        case "enable":
          ...
          respond({
            text: `:white_check_mark: Joins enabled. Don't forget to \`/allowjoins link\` if you want people to find it via your name!`,
            response_type: "ephemeral",
          });
          break;
