              ...
              elements: [
                {
                  ...
                  value: JSON.stringify({ requesterId, channelId: targetChannelId })
                },
                {
                  type: "button",
    ...
    app.command("/join", async ({ command, ack, respond, client }) => {
      ...
      if (config.questions && config.questions.length > 0) {
          try {
              ...
              return respond({ text: ":x: Failed to open questionnaire.", response_type: "ephemeral" });
          }
      }
    ...
    app.command(this.name, async ({ command, ack, respond }) => {
      ...
      switch (action) {
        ...
        case "enable":
          ...
          respond({ text: `:white_check_mark: Joins enabled.`, response_type: "ephemeral" });
          break;
        ...
        case "questions":
            ...
            if (qAction === "list") {
                if (config.questions.length === 0) {
                    return respond({ text: "No questions configured.", response_type: "ephemeral" });
                }
                const list = config.questions.map((q, i) => `${i + 1}. ${q}`).join("\n");
                respond({ text: `*Current Questions:*\n${list}`, response_type: "ephemeral" });
            } 
            else if (qAction === "add") {
                if (!qText) return respond({ text: "Please provide question text.", response_type: "ephemeral" });
                if (config.questions.length >= 5) return respond({ text: "Maximum 5 questions allowed.", response_type: "ephemeral" });
                
                // Remove quotes if user wrapped in them
                let cleanQ = qText;
                if ((cleanQ.startsWith('"') && cleanQ.endsWith('"')) || (cleanQ.startsWith('“') && cleanQ.endsWith('”'))) {
                    cleanQ = cleanQ.slice(1, -1);
                }
                
                config.questions.push(cleanQ);
                await app.dbs.joinchanneldb.set(command.channel_id, config);
                respond({ text: `Added question: "${cleanQ}"`, response_type: "ephemeral" });
            }
            else if (qAction === "clear") {
                config.questions = [];
                await app.dbs.joinchanneldb.set(command.channel_id, config);
                respond({ text: "Cleared all questions.", response_type: "ephemeral" });
            }
            else {
                respond({ text: "Usage: `/allowjoins questions [list|add <text>|clear]`", response_type: "ephemeral" });
            }
            break;
