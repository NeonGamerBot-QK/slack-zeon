    ...
    app.event(this.name, async (par) => {
      ...
      try {
        ...
        try {
          ...
          aiReq0 =
            typeof aiReq00 === "string" ? aiReq00 : JSON.stringify(aiReq00);
        }
        ...
        // Hardcoded slur filter - block banned words
        const bannedWords = [
          /\br[e3][t]+[a@4][r]+d[e3]?[d]?\b/gi,
          /\bf[a@4][g]+[o0][t]+\b/gi,
          /\bn[i1][g]+[e3][r]+\b/gi,
        ];
        ...
        if (aiReq.message) {
          aiReq.message = filterSlurs(aiReq.message);
        }
        if (aiReq.comment) {
          aiReq.comment = filterSlurs(aiReq.comment);
        }
        if (aiReq.error) {
          aiReq.error = filterSlurs(aiReq.error);
        }
