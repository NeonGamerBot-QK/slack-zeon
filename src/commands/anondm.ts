...
const users =  theViewState.find(e=>e["multi_users_select-action"])["multi_users_select-action"].selected_users
const textBox = theViewState.find(e=>e["plain_text_input-action"])["plain_text_input-action"].value
...
for(const user of users) {
  ...
  if(usersInDb.find(e=>bcrypt.compareSync(user, e))) {
    if(app.dbs.anondm.get(usersInDb.find(e=>bcrypt.compareSync(user, e))).messages.find(e=>{
      try {
        EncryptedJsonDb.decrypt(e.message, `${user}_`+process.env.ANONDM_PASSWORD)
      return true;
      } catch(e) {
        return false;
      }
...
for(const u of ac_user_list) {
  ...
  let user_id = null;
    ...
    if(userInDb) {
      ...
      // userProfile = 
      userProfile.messages.push(EncryptedJsonDb.encrypt(JSON.stringify({
        message: textBox,
